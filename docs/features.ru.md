...
- **Отменённый прогон тоже считается.** Если у PR нет чистого завершённого RunAll, но есть
  отменённый, успевший прогнать большинство сьют, чекер анализирует этот (частичный) прогон —
  баннер *RunAll interrupted* и его реальные падения — вместо «прогонов нет». Чистый завершённый
  прогон всегда в приоритете, так что свежая отмена не затеняет хороший вердикт.
