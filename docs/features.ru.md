...
- **Авто-перепрогон блокеров** (настройки, независимо от визы) — когда твой RunAll финиширует с
  блокерами, их сьюты перепрогоняются автоматически, до 2 попыток: ≤10 сьют встают в начало очереди,
  больше — в хвост, чтобы не мешать остальным; системная поломка (30+) не трогается. Одноимённые
  сьюты, уже висящие в очереди, предварительно отменяются. Прошедший перепрогон снимает свой блокер;
  если включена и виза — она ждёт, пока перепрогоны устаканятся.
- **Комментарий в GitHub-PR** (настройки, независимо от двух других) — тот же вердикт в
  ...
  сообщений.
- **`/run-all` из PR** — при включённой GitHub-опции комментарий `/run-all` в любом pull request
  ставит всю цепочку RunAll в очередь под твоим собственным TeamCity-токеном; подтверждение —
  реакция 🚀 плюс ссылка на поставленный TC-билд, дописанная **в сам командный комментарий**
  (😕-реакция — если TeamCity отказал) — по-прежнему ноль лишних сообщений. Там же живёт строка
  **«~Xh Ym remaining — ≈ 21:05 MSK»** (queue-aware; абсолютное время — в таймзоне твоего
  JIRA-профиля; обновляется каждые ~5 минут) до самого финиша. Топ — часть команды:
  **`/run-all top`** сразу ставит цепочку в начало очереди, а пока твоя командная цепочка ждёт,
  ответ **`/top`** поднимает её — только её, чужие билды не трогаются. Команда от ещё не
  подключённого пользователя получает разовый ответ — куда зайти и какую галку включить: команда
  сама себя продаёт. Вместе с
  авто-перепрогоном и вердиктом-комментарием весь цикл проходит, не выходя из PR. Команды
  подхватываются в течение минуты — один опрос комментариев на весь репозиторий покрывает все PR.
