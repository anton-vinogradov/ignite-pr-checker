...
- **Комментарий в GitHub-PR** (настройки, независимо от двух других) — тот же вердикт в
  GitHub-markdown постится комментарием в pull request `apache/ignite` от твоего собственного
  GitHub-аккаунта (personal access token со скоупом `public_repo`; пока опция включена, хранится
  зашифрованным).
- **`/run-all` из PR** — при включённой GitHub-опции комментарий `/run-all` в любом pull request
  ...
  реакция 🚀 от твоего же аккаунта (😕 — если TeamCity отказал). Вместе с авто-перепрогоном и
  вердиктом-комментарием весь цикл проходит, не выходя из PR. Команды подхватываются в течение
  минуты — один опрос комментариев на весь репозиторий покрывает все PR.
