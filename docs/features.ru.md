...
- **`/run-all` из PR** — при включённой GitHub-опции комментарий `/run-all` в любом pull request
  ...
  **«~Xh Ym remaining — ≈ 21:05 MSK»** (queue-aware; абсолютное время — в таймзоне твоего
  JIRA-профиля; обновляется каждые ~5 минут) до самого финиша. Пока цепочка ждёт в очереди,
  ответ **`/top`** поднимает твои queued-билды этого PR в начало очереди. Вместе с
